...
TEST(MatrixCopyTest, CopyConstructor) {
	...
	boost::array< boost::multi_array_types::index, 2 > extents = {{ 2, 2 }};
	matrix1.initialize(extents[0], extents[1], 15);
...
TEST(DataSetTest, SingleValue) {
	...
	HeatFlow::MatrixFile<int> *test_file = new HeatFlow::MatrixFile<int>(2, 2);
	boost::array< boost::multi_array_types::size_type, 2 > extents = {{ 3, 4 }};
	boost::multi_array<int,2> sample_data(extents);
	test_file->set_data(sample_data);
	...
	EXPECT_EQ(3, test_file->get_data()->shape()[0]);
	EXPECT_EQ(4, test_file->get_data()->shape()[1]);
	EXPECT_EQ(3, test_file->get_size1());
	EXPECT_EQ(4, test_file->get_size2());
...
TEST(FileIOTest, ReadIntAsciiSuccess) {
	...
	EXPECT_EQ(10, test_file->get_size1());
	EXPECT_EQ(15, test_file->get_size2());
