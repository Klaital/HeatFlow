...
TEST(ProjectAccessorsTest, SingleMaterialQuery) {
	...
	HeatFlow::MatrixFile<HeatFlow::Material *> materials;
	project.copy_materials_matrix(materials);
	HeatFlow::Material *m = materials.get_datum(0, 0);
	EXPECT_EQ("Air", m->get_name());
	EXPECT_EQ(1.225, m->get_density());
	EXPECT_EQ(0.024, m->get_conductivity());

	m = materials.get_datum(2, 2);
	EXPECT_EQ("Aluminum", m->get_name());
	EXPECT_EQ(2712, m->get_density());
	EXPECT_EQ(205, m->get_conductivity());
}